...
# SSL
mkdir /etc/nginx/ssl
openssl req -newkey rsa:4096 -x509 -sha256 -days 365 -nodes -out /etc/nginx/ssl/monsupersite.pem -keyout /etc/nginx/ssl/monsupersite.key -subj "/C=FR/ST=Paris/L=Paris/O=42 School/OU=mprigent/CN=monsupersite"
...
if [ "$AUTOINDEX" = "off" ]; then
	sed -i "s/autoindex on/autoindex off/g" /etc/nginx/sites-available/monsupersite;
#else if [ "$AUTOINDEX" = "on" ]; then
else
	sed -i "s/autoindex off/autoindex on/g" /etc/nginx/sites-available/monsupersite;
fi
